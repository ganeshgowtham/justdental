...
/*
var $table = $('#patientTable').bootstrapTable({
    data: patientData
});

$table.on('click-cell.bs.table', function(e, value, row, $element){
   // alert( "Valsue: " + value );console.log(row)
    for(var i = $table.data('bootstrap.table').data.length; i--;) {
        
          if($table.data('bootstrap.table').data[i].name == row) {
              $table.data('bootstrap.table').data.splice(i, 1);
             
          }else {
               console.log(i, $table.data('bootstrap.table').data[i].name, row)
          }
      }
